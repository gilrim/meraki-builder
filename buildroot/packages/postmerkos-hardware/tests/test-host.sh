...
LED_ENV="POSTMERKOS_RUN_DIR=$TMP/run POSTMERKOS_HWPROBE=$ROOT/files/postmerkos-hwprobe POSTMERKOSCTL=$TMP/bin/postmerkosctl POSTMERKOS_HARDWARE_PROFILE_DIR=$TMP/profiles POSTMERKOS_BOARDINFO=$TMP/boardinfo POSTMERKOS_CLICK_ROOT=$TMP/click POSTMERKOS_SYS_ROOT=$TMP/sys POSTMERKOS_PROC_INPUT=$TMP/proc-input POSTMERKOS_DEV_INPUT=$TMP/dev/input"
...
grep -q '^reset-progress 100$' "$TMP/led-calls"
python3 - "$TMP/run/button-status.json" <<'PY'
import json, sys
p=json.load(open(sys.argv[1]))
