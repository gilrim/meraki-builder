...
By default it fetches and resets each checkout to `origin/main` immediately
before use. `LOADER_REF=latest` remains accepted only as a compatibility alias
...
when current `main` and the builder are out of sync.
