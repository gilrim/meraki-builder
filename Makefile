...
.PHONY: help all base web doctor deps sources kernel loader donor ui prepare rootfs image \
        validate verify-inputs test-fwupdate test-image test-modules test-hardware test-docs test-ui-contract test-ui-build verify-modules test-all menuconfig distrobox clean distclean print-config \
...
help:
	@printf '%s\n' \
	  ...
	  '  make test-ui-build Verify npm lockfile registry and timeout safeguards' \
...
test-ui-build:
	@./scripts/tests/test-ui-npm-lock.py

test-all: test-fwupdate test-image test-modules test-hardware test-docs test-ui-contract test-ui-build
